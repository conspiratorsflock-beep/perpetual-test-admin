...
export async function getProjectTestCases(
  ...
  const testCases = (data || []).map((row) => ({
    ...
    automationStatus: row.automation_status as "not_automated" | "automated" | "partial",
    executionMode: row.execution_mode as "manual" | "automated",
...
export async function getTestCaseById(id: string): Promise<TestCase | null> {
  ...
  return {
    ...
    automationStatus: data.automation_status as "not_automated" | "automated" | "partial",
    executionMode: data.execution_mode as "manual" | "automated",
